...
Version 0.34 · Updated 21 September 2026 · Owner: Founder · Maintainer: project engineering assistant

**Current session authority: HARVO-034 — rollout preparation, subsequently restricted to local execution.** Founder accepts the six HARVO-033 local implementation workstreams, endorses the paid-pool circuit breaker and authorizes local/staged signing-key preparation. The subsequent explicit instruction requires the 027–031 migration/grant rollback rehearsal on disposable local PostgreSQL only: do not connect to remote endpoints in `.env` or `.env.local`; the founder will prepare `.env.staging.local` separately. Dedicated product separation is verified, but actual activation still requires accepted canonical booking/current-consent composition and external operating evidence. Follow [HARVO-034 rollout plan](docs/implementation/HARVO_034_ROLLOUT_PLAN.md). No production/pilot acceptance is inferred from executive recognition or configuration strings.
