...
const regex = /async function processPaymentWebhook\(payload: any, signature: string \| undefined, req: any\) \{([\s\S]*?)return \{ success: true, message: 'Webhook processed successfully' \};\n\}/m;

const match = code.match(regex);
if (!match) {
   console.log("Could not find processPaymentWebhook");
   process.exit(1);
}

const replacement = `async function processPaymentWebhook(payload: any, signature: string | undefined, req: any) {
...
console.log('Webhook processor updated successfully');
