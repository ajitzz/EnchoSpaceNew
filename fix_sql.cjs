...
server = server.replace(
    /queryStr \+= \` AND \(l\.bedrooms >= \$\{queryParams\.length\} OR EXISTS \(SELECT 1 FROM jsonb_array_elements\(l\.rooms\) as r WHERE \(r->>'bedrooms'\)::numeric >= \$\{queryParams\.length\}\)\)\`;/g,
...
server = server.replace(
    /queryStr \+= \` AND \(l\.beds >= \$\{queryParams\.length\} OR EXISTS \(SELECT 1 FROM jsonb_array_elements\(l\.rooms\) as r WHERE \(r->>'beds'\)::numeric >= \$\{queryParams\.length\}\)\)\`;/g,
...
server = server.replace(
    /queryStr \+= \` AND \(l\.max_guests >= \$\{queryParams\.length\} OR EXISTS \(SELECT 1 FROM jsonb_array_elements\(l\.rooms\) as r WHERE \(r->>'capacity'\)::numeric >= \$\{queryParams\.length\}\)\)\`;/g,
...
console.log('Fixed SQL injection/parameter issue');
