...
export async function projectOperationsWorkspace(client: pg.PoolClient, principal: PrincipalContext,
  ...
  const rows = (await client.query<{permission_code: string}>(`SELECT DISTINCT p.permission_code
    FROM internal_membership_grants g JOIN internal_role_permissions p ON p.role_version_id=g.role_version_id
