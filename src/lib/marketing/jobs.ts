...
export async function enqueue(c:pg.PoolClient|pg.Pool,input:{campaignId?:number;revision?:number;kind:JobKind;key:string;payload?:Record<string,unknown>}){
 const payload=input.payload||{};
 const r=await c.query(`INSERT INTO marketing_jobs(id,campaign_id,revision,kind,dedupe_key,payload) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(dedupe_key) DO NOTHING RETURNING id`,[randomUUID(),input.campaignId??null,input.revision??null,input.kind,input.key,JSON.stringify(payload)]);
...
export class MarketingJobQueue{
 ...
 async claim():Promise<MarketingJob|null>{
  await this.pool.query(`UPDATE marketing_jobs SET state=CASE WHEN kind IN ('PUBLISH','ACTIVATE') THEN 'RECONCILIATION_REQUIRED' ELSE 'RETRY' END, fence=fence+1,last_error='WORKER_LEASE_EXPIRED',lease_until=NULL,run_after=now()+interval '10 seconds',updated_at=now() WHERE state='RUNNING' AND lease_until<now()`);
 ...
 async fail(job:MarketingJob,code:string,uncertain=false){
  const mutation=job.kind==='PUBLISH'||job.kind==='ACTIVATE';const state=uncertain||mutation?'RECONCILIATION_REQUIRED':job.attempts>=8?'DEAD':'RETRY';
