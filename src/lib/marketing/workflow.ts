import type {CampaignStrategyBindings} from './adtech/bindings.js';
...
export interface WorkflowOptions{strategies?:CampaignStrategyBindings;resolveStory?:(c:pg.PoolClient,actor:Actor,draft:CampaignDraft)=>Promise<NonNullable<ListingEvidence['spatialStory']>>;guardDedicated?:(c:pg.PoolClient,row:any)=>Promise<void>;bindProduct?:(c:pg.PoolClient,actor:Actor,listing:ListingEvidence)=>Promise<NonNullable<ListingEvidence['marketingProduct']>>;verifyProduct?:(c:pg.PoolClient,actor:Actor,product:NonNullable<ListingEvidence['marketingProduct']>)=>Promise<unknown>;ai:CampaignAiReviewer;finance:WorkflowFinancePort;publishingEnabled:boolean;activationEnabled:boolean;fundingEnabled:boolean;configurationReasons:string[];evaluationTimeoutMs?:number;validateTargeting?:(draft:CampaignDraft)=>Promise<void>;metaCountries?:readonly string[];activationBlockers?:(provider:'GOOGLE'|'META')=>string[];resolveCreative?:(c:pg.PoolClient,actor:Actor,draft:CampaignDraft,listing:ListingEvidence)=>Promise<CampaignCreativeEvidence>;verifyCreativeForPublishing?:(row:any,actor:Actor)=>Promise<void>;}
...
export class MarketingWorkflowService{
 ...
 async get(id:number,actor:Actor){await this.recoverExpiredEvaluations(actor,id);return inTransaction(this.pool,actor,async c=>{const r=await c.query('SELECT * FROM marketing_campaign_workflows WHERE campaign_id=$1 AND ($2 OR host_id=$3)',[id,actor.role==='admin'||actor.role==='system',actor.id]);if(!r.rows[0])throw new MarketingError('CAMPAIGN_NOT_FOUND','Campaign not found',404);const row=r.rows[0];if(row.draft.strategySelection){if(!this.options.strategies)throw new MarketingError('STRATEGY_NOT_CONFIGURED','Strategy compiler is unavailable.',503);row.adtechStrategy=await this.options.strategies.load(c,row);}return row;});}
 ...
 private async saveRevision(c:pg.PoolClient,row:any){await c.query(`INSERT INTO marketing_campaign_revisions(campaign_id,revision,host_id,draft,listing_snapshot,listing_hash) VALUES($1,$2,$3,$4,$5,$6)`,[row.campaign_id,row.revision,row.host_id,row.draft,row.listing_snapshot,row.listing_hash]);
  if(row.draft.strategySelection&&!this.options.strategies)throw new MarketingError('STRATEGY_NOT_CONFIGURED','Strategy registry is unavailable.',503);
  await this.options.strategies?.bind(c,row);
 ...
 async evaluate(id:number,actor:Actor,revision:number){
  ...
  try{result=await Promise.race([this.options.ai.evaluate(row.draft,this.aiListing(row),row.revision,row.adtechStrategy?.profile.ai.minimumScore,row.adtechStrategy),new Promise<AiEvaluation>(resolve=>{timer=setTimeout(()=>resolve(this.reviewFallback(row,'AI review exceeded its time limit. No automatic pass was issued; retry evaluation or complete documented human review.')),this.evaluationTimeoutMs);})]);}
 ...
 async requestObservation(id:number,actor:Actor,expectedRevision?:unknown){
  return inTransaction(this.pool,actor,async c=>{
   const row=await lockWorkflow(c,id,actor);requireRevision(row,expectedRevision===undefined?row.revision:expectedRevision);const revision=row.revision;
   if(!row.provider_truth?.externalCampaignId)throw new MarketingError('PROVIDER_ID_MISSING','This campaign has not been submitted to an advertising network.');
   const latest=(await c.query("SELECT id,state,attempts,run_after,updated_at,created_at FROM marketing_jobs WHERE campaign_id=$1 AND revision=$2 AND kind='TELEMETRY' ORDER BY created_at DESC,id DESC LIMIT 1",[id,revision])).rows[0];
   if(latest&&(['PENDING','RUNNING','RETRY'].includes(latest.state)||Date.now()-new Date(latest.created_at).getTime()<60000))return {status:latest.state,jobId:latest.id,coalesced:true};
   const jobId=await enqueue(c,{campaignId:id,revision,kind:'TELEMETRY',key:`refresh:${id}:${revision}:${Math.floor(Date.now()/60000)}`});
 ...
 async workspace(actor:Actor,input:WorkspaceQuery={}){
  ...
  const data=await inTransaction(this.pool,actor,async c=>{
   const all=(await c.query(`SELECT w.*,(SELECT jsonb_build_object('id',j.id,'state',j.state,'attempts',j.attempts,'updated_at',j.updated_at,'run_after',j.run_after) FROM marketing_jobs j WHERE j.campaign_id=w.campaign_id AND j.revision=w.revision AND j.kind='TELEMETRY' ORDER BY j.created_at DESC,j.id DESC LIMIT 1) AS observation_job FROM marketing_campaign_workflows w WHERE ($1 OR host_id=$2) AND ($3::bigint IS NULL OR campaign_id<$3) AND ($4='' OR draft->>'title' ILIKE $5 ESCAPE E'\\\\' OR listing_snapshot->>'title' ILIKE $5 ESCAPE E'\\\\' OR campaign_id::text=$4) AND ($6='all' OR $6='review' AND state='PENDING_ADMIN' OR $6='exceptions' AND (last_error IS NOT NULL OR state IN ('FAILED','RECONCILIATION_REQUIRED','AI_REJECTED','ADMIN_REJECTED'))) ORDER BY campaign_id DESC LIMIT 31`,[actor.role==='admin',actor.id,query.before??null,query.search,containsPattern(query.search),query.filter])).rows;
  ...
  return {listings:data.listings,campaignListings:data.campaignListings,campaigns:await Promise.all(data.rows.map(r=>this.project(r,finances?.get(r.campaign_id),actor.role==='admin'?'admin':'host'))),policy:this.options.finance.policy(),capabilities:{adtech:this.options.strategies?.required===true,funding:this.options.fundingEnabled,publish:this.options.publishingEnabled,activate:this.options.activationEnabled,reason:this.options.configurationReasons.join(' '),metaCountries:[...(this.options.metaCountries??[])]},page:{limit:30,mayHaveMore:data.nextCursor!==null,nextCursor:data.nextCursor,order:'CREATED_NEWEST'},listingPage:{limit:50,mayHaveMore:data.listingNextCursor!==null,nextCursor:data.listingNextCursor}};
 }
}
