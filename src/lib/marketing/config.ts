...
export function readMarketingConfig(raw=process.env.HARVO_MARKETING_CONFIG):MarketingRuntimeConfig{
 if(!raw?.trim())return {origin:process.env.GOOGLE_ADS_LANDING_ORIGIN||'',mediaOrigins:[],currency:'INR',markupBps:500,financialPolicy:null,policyAdminId:null,costRules:[],remittanceTax:{base:'NONE',rateBps:0},fundingEnabled:false,publishingEnabled:false,activationEnabled:false,providerClearance:{},checkoutAcceptanceReference:null,meta:{countries:[],placements:[],specialAdCategories:[]}};
 try{const c=schema.parse(JSON.parse(raw));c.origin=publicOrigin(c.origin);c.mediaOrigins=c.mediaOrigins.map(x=>publicOrigin(x));if(c.settlement?.documentOrigins)for(const origins of Object.values(c.settlement.documentOrigins))if(origins)origins.forEach((origin,index)=>{origins[index]=publicOrigin(origin);});validateCostPolicy(c.financialPolicy);if(c.financialPolicy.currency!==c.currency)throw new Error();if(c.activationEnabled&&!c.checkoutAcceptanceReference)throw new Error();if((c.publishingEnabled||c.activationEnabled)&&c.providerClearance.META&&(!c.meta.countries.length||!c.meta.placements.length))throw new Error();return c as MarketingRuntimeConfig;}catch{throw new MarketingError('MARKETING_CONFIG_INVALID','Marketing operator configuration is invalid. Check the documented schema without exposing credentials.',503);}
}
