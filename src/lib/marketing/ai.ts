...
export function normalizeAiEvaluation(value:unknown,context:Omit<AiEvaluation,'status'|'score'|'notes'|'suggestions'>):AiEvaluation{
 const result=evaluationSchema.parse(value);return {...context,status:result.verdict==='REVIEW'?'REQUIRES_REVIEW':result.verdict==='REJECT'||result.score<8?'REJECTED':'PASSED',score:result.score,notes:result.notes,suggestions:result.suggestions};
}
export class CampaignAiReviewer{
 ...
 async evaluate(draft:CampaignDraft,listing:ListingEvidence,revision:number):Promise<AiEvaluation>{
  ...
  try{
   ...
   return normalizeAiEvaluation(JSON.parse(text||''),context);
