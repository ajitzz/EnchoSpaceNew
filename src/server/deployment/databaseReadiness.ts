...
export async function databaseReadiness(pool:pg.Pool){
 const c=await pool.connect();try{await c.query('BEGIN READ ONLY');await c.query("SET LOCAL statement_timeout='3000ms'");
  ...
  await c.query('COMMIT');return {ready:role?.rolsuper===false&&role?.rolbypassrls===false&&!missing.length&&forcedRls&&migrationsValid&&recovery.ready&&portfolio.ready,missing,roleBypassesRls:!!(role?.rolsuper||role?.rolbypassrls),forcedRls,migrationsValid,recovery,portfolio};
