...
export async function verifyPortfolioCatalog(c:pg.PoolClient){
 ...
 const privileges=sequenceRows.length===sequences.length&&sequenceRows.every(s=>s.usage&&!s.update&&!s.owns)&&grants.length===portfolioTables.length&&grants.every(g=>g.relrowsecurity&&g.relforcerowsecurity&&!g.owns&&g.can_read&&g.can_insert&&!g.can_truncate&&!g.can_trigger&&!g.public_grant&&g.can_update===mutableTables.includes(g.relname)&&g.can_delete===(g.relname==='marketing_keyword_research'||expiringTables.includes(g.relname)));
