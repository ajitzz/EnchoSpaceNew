...
export async function verifyConversationCatalog(client:pg.PoolClient,mode:'runtime'|'worker'='runtime'){
 ...
 const isOwner=tableRows.some(row=>row.owns);
 const tableSafety=tableRows.length===5&&tableRows.every(row=>row.relrowsecurity&&row.relforcerowsecurity&&(isOwner||(!row.owns&&!row.public_grant)));
 ...
 const exactPolicies=contracts.filter(([,,,,kind])=>isOwner?(kind==='public'||kind==='runtime'):true).every(([table,name,cmd,permissive,kind,qual,check])=>policies.some(row=>row.tablename===table&&row.policyname===name&&row.cmd===cmd&&row.permissive===permissive&&(isOwner||(row.roles.length===1&&row.roles[0]===expectedRole[kind]))&&normalized(row.qual)===normalized(qual)&&normalized(row.with_check)===normalized(check)));
 const namedRoles=isOwner||Boolean(runtimePolicyRole&&workerPolicyRole&&runtimePolicyRole!=='public'&&workerPolicyRole!=='public'&&runtimePolicyRole!==workerPolicyRole&&expectedRole[mode]===role?.name);
