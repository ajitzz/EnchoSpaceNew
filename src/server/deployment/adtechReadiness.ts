...
export async function verifyAdtechCatalog(c:pg.PoolClient){
 ...
 const privileges=grants.length===adtechTables.length&&grants.every(g=>g.relrowsecurity&&g.relforcerowsecurity&&(isOwner||!g.owns)&&g.r&&(isOwner||(!g.truncate&&!g.trigger&&!g.public_grant&&(g.i===(!['marketing_adtech_current_release','marketing_corridor_inference_policy'].includes(g.relname)))&&(g.u===(mutable.includes(g.relname)||g.relname==='marketing_adtech_current_release'))&&(g.d===(g.relname==='marketing_corridor_inference_rate')))))&&sequenceRows.length===adtechSequences.length&&sequenceRows.every(s=>s.usage&&(isOwner||(!s.update&&!s.owns)));
