...
export const legacyMarketingBoundary:RequestHandler=(req,res,next)=>{
 ...
 if(campaignMutation||dangerousOther)return res.status(410).json({error:'This legacy paid-marketing operation has been retired. Open the campaign studio to use revision-bound review, verified funding and provider controls.',code:'HARVO_V2_REQUIRED'});
 next();
};
