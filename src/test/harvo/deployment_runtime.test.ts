...
describe('truthful structural readiness',()=>{
 ...
 it('rejects missing migrations, bypass roles and disabled FORCE RLS despite a working connection',async()=>{for(const config of[{missing:'marketing_jobs'},{bypass:true},{force:false}])expect((await databaseReadiness(pool(config) as any)).ready).toBe(false);expect((await databaseReadiness(pool() as any)).ready).toBe(false); /* Tables alone cannot prove catalog or migration integrity. */});
