...
import {prepareRestoreSettlement,prepareRestoreConversion,restoreAdmin,restoreHost,restoreSettlementOptions,restoreWorkflow} from './gapRestoreFixture.js';
...
describe('current financial and conversion evidence survives real PostgreSQL dump/restore',()=>{
 ...
 afterAll(async()=>{await fixture?.close();const directory=new URL('../../../docs/harvo/artifacts',import.meta.url);mkdirSync(directory,{recursive:true});writeFileSync(new URL('gap-restore.json',directory.href.endsWith('/')?directory:new URL(directory.href+'/')),JSON.stringify(report,null,2)+'\n');});
 ...
 it('preserves exact bytes, settled accounting, unknown dispatch ownership and nonowner access barriers',async()=>{
  ...
  try{
   ...
   const after=await digest(restored);expect(after).toEqual(before);expect(await schema(restored)).toEqual(beforeSchema);expect(beforeSchema.every(row=>row.relrowsecurity&&row.relforcerowsecurity)).toBe(true);
