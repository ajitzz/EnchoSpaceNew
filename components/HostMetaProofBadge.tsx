...
export const HostMetaProofBadge: React.FC<HostMetaProofBadgeProps> = ({ proof }) => {
  ...
  const copySignature = () => {
    navigator.clipboard.writeText(proof.cryptographic_verification_signature).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    }).catch(error => console.error('Could not copy the provider record:', error));
  };
    ...
    <div className="p-4 rounded-2xl bg-zinc-900 dark:bg-black border border-zinc-800 text-white shadow-md relative overflow-hidden">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-300 mt-0.5">
              Stored network identifiers and the last recorded verification time.
            </p>
          </div>
        </div>

        {proof.cryptographic_verification_signature && <button
      ...
      <div className="mt-3 pt-3 border-t border-zinc-800/80 grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px] text-zinc-400 font-mono">
        <div>
          ...
          <span className="text-zinc-200">{proof.meta_campaign_id || 'Not recorded'}</span>
        </div>
        <div>
          <span className="text-zinc-500 block text-[10px]">AdSet ID</span>
          <span className="text-zinc-200">{proof.meta_adset_id || 'Not recorded'}</span>
        </div>
        <div>
          <span className="text-zinc-500 block text-[10px]">Verified At</span>
          <span className="text-zinc-200">{Number.isFinite(Date.parse(proof.verified_at)) ? new Date(proof.verified_at).toLocaleString() : 'Not recorded'}</span>
        </div>
      </div>
    </div>
  );
