        ...
        <div className="w-full md:max-w-7xl mx-auto px-4 md:px-6 lg:px-8 mt-4 md:mt-6 flex flex-col gap-16 md:gap-24">
          ...
          <section className="space-y-8 pt-8 border-t border-zinc-200/80">
            ...
            <div className="md:hidden space-y-2.5">
              ...
              {/* 2. Ergonomic Mobile Control Dock (Below Slide Track with Center 1/4 Tracker & Right Book CTA) */}
              <div className="bg-zinc-100/90 backdrop-blur-md px-3.5 py-2 rounded-2xl border border-zinc-200/70 shadow-2xs flex items-center justify-between gap-2">
                {/* Left: Active Room Tier Name */}
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="w-1.5 h-1.5 rounded-full bg-zinc-900 shrink-0" />
                  <span className="text-xs font-bold font-display text-zinc-800 tracking-tight truncate max-w-[130px]">
                    {activeSlide === 0 ? 'Presidential Suites' : activeSlide === 1 ? 'Deluxe Double Rooms' : 'Executive Single Rooms'}
                  </span>
                </div>

                {/* Center: Slide Progress Dots + 1/4 Counter */}
                <div className="flex items-center gap-1.5 shrink-0 bg-white px-2.5 py-1 rounded-full border border-zinc-200/60 shadow-2xs">
                  {[1, 2, 3, 4].map((step) => {
                  ...
                  <span className="text-[10px] font-bold text-zinc-700 font-mono ml-0.5">
                    {(mobileSpaceIndex % 4) + 1}/4
                  </span>
                </div>

                {/* Right: Instant Book Action Button */}
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className="bg-zinc-900 hover:bg-zinc-800 text-white text-xs font-extrabold px-3.5 py-1.5 rounded-xl shadow-xs active:scale-95 transition-all flex items-center gap-1 shrink-0 cursor-pointer"
                >
