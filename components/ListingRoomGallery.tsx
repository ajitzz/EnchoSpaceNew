...
export function ListingRoomGallery({listing,onOpen}:{listing:Listing;onOpen:(tier:string,index:number)=>void}) {
  return <section className="space-y-8" aria-label="Rooms and room photography">
    ...
    {(listing.rooms || []).map(room => {
      ...
      const photos=(listing.photos || []).filter(photo => photo.tier===tier && !!photo.url);
      const roomPhotos:SpatialPhoto[]=photos.length ? photos : (room.photos || []).filter(photo=>!!photo.url);
      return <article key={room.id} className="overflow-hidden rounded-3xl bg-white border border-zinc-200">
        <div className="p-6 md:p-8 flex flex-wrap justify-between gap-4">
          ...
          {Number.isFinite(room.price) && room.price>0 && <p className="text-lg font-semibold">From {new Intl.NumberFormat('en-IN',{style:'currency',currency:listing.currency || 'INR',maximumFractionDigits:0}).format(room.price)} <span className="text-sm text-zinc-500">/ night</span></p>}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2">
          {roomPhotos.slice(0,4).map((photo,index) => <button type="button" key={photo.id || photo.url} className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-zinc-100 text-left group" aria-label={`View ${room.name} photo ${index+1}`} onClick={()=>onOpen(tier,index)}>
