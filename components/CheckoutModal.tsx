...
const CheckoutForm = ({ amount, onPaymentSuccess, onCancel }: { amount: number, onPaymentSuccess: () => void, onCancel: () => void }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <button disabled={processing || !stripe} type="submit" className="w-full py-3.5 bg-zinc-900 hover:bg-zinc-800 text-white font-bold rounded-xl transition-all shadow-lg disabled:opacity-50 text-sm">
        {processing ? 'Processing...' : `Pay ₹${amount.toLocaleString()}`}
      </button>
      <button type="button" onClick={onCancel} className="w-full py-2 text-zinc-500 font-semibold hover:text-zinc-900 transition-colors text-xs">
        Cancel & Go Back
      </button>
    </form>
  );
...
export const CheckoutModal = ({ amount: baseAmount, isOpen, onClose, onSuccess }: { amount: number, isOpen: boolean, onClose: () => void, onSuccess: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
        <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden relative border border-zinc-100 flex flex-col md:flex-row h-auto max-h-[90vh]">
            ...
            <div className="w-full md:w-[42%] bg-zinc-50 p-6 flex flex-col justify-between border-b md:border-b-0 md:border-r border-zinc-100 overflow-y-auto">
               ...
               <div className="mt-8 pt-4 border-t border-zinc-200/60">
                  <div className="flex justify-between items-baseline mb-2">
                     <span className="text-xs font-bold text-zinc-900 uppercase tracking-widest">Total cost</span>
                     <span className="text-2xl font-extrabold text-zinc-900 tracking-tight font-mono">₹{finalTotal.toLocaleString()}</span>
                  </div>
            ...
            <div className="w-full md:w-[58%] p-6 md:p-8 overflow-y-auto flex flex-col justify-between">
                ...
                {isSimulating && (
                   <div className="absolute inset-0 bg-white/95 backdrop-blur-xs z-50 flex flex-col items-center justify-center p-8 text-center animate-fade-in">
                      ...
                      <h4 className="text-lg font-bold text-zinc-900 mb-2">Processing Transaction</h4>
                      
                      <div className="space-y-2 max-w-xs">
                         <p className={`text-sm transition-all duration-300 font-medium ${simulationStep >= 1 ? 'text-zinc-900' : 'text-zinc-300'}`}>
                            {simulationStep >= 1 ? '✓ Initiating secure gateway connection' : 'Connecting to gateway...'}
                         </p>
                         <p className={`text-sm transition-all duration-300 font-medium ${simulationStep >= 2 ? 'text-zinc-900 font-bold text-[#0284C7]' : 'text-zinc-300'}`}>
                       ...
                       <div className="animate-fade-in space-y-6">
                          <div className="flex gap-4 border-b border-zinc-100 pb-3">
                             <button 
                                ...
                                className={`text-xs font-bold pb-2 border-b-2 transition-all ${razorpayMethod === 'upi' ? 'border-[#0284C7] text-zinc-900' : 'border-transparent text-zinc-400 hover:text-zinc-900'}`}
                             >
                                UPI / QR / Netbanking
                             </button>
                             <button 
                                onClick={() => setRazorpayMethod('emi')}
                                className={`text-xs font-bold pb-2 border-b-2 transition-all flex items-center gap-1.5 ${razorpayMethod === 'emi' ? 'border-[#0284C7] text-zinc-900' : 'border-transparent text-zinc-400 hover:text-zinc-900'}`}
                             >
                             ...
                             <div className="space-y-4 animate-fade-in">
                                <div className="space-y-1">
                                   ...
                                   <input 
                                      ...
                                      className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#0284C7] focus:outline-none transition-all font-mono text-sm"
                                   />
                                </div>
                                ...
                                <div className="flex items-center gap-3 p-3 bg-zinc-50 rounded-2xl border border-zinc-200/50">
                                   ...
                                   <div>
                                      <p className="text-xs font-bold text-zinc-900">Direct App Redirection</p>
                             ...
                             <div className="space-y-4 animate-fade-in">
                                      ...
                                      {EMI_BANKS.map((bank) => (
                                         ...
                                         >
                                            ...
                                            <div className="leading-none">
                                               <p className="text-[10px] font-extrabold text-zinc-900 truncate max-w-[80px]">{bank.name}</p>
                                   ...
                                   <div className="flex gap-2">
                                      {EMI_TENURES.map((months) => {
                                         ...
                                         return (
                                            ...
                                            >
                                               <span className="text-xs font-extrabold text-zinc-900">{months} Months</span>
                                ...
                                <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-200/50 space-y-3.5">
                                   <div className="flex items-center justify-between text-xs font-medium text-zinc-600">
                                      ...
                                      <span className="font-extrabold text-zinc-900 text-sm font-mono">₹{emiDetails.monthly} <span className="text-[10px] font-normal text-zinc-400">/ mo</span></span>
                                   </div>
                                   <div className="flex items-center justify-between text-xs font-medium text-zinc-600">
                                      ...
                                      <span className="font-semibold text-zinc-900 font-mono">₹{emiDetails.interest}</span>
                                   </div>
                                   <div className="border-t border-zinc-200/60 pt-3 flex items-center justify-between text-xs font-bold text-zinc-900">
