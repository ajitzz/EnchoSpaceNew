...
const oldRoomsMapStart = `{listing.rooms.map((room, idx) => {
                                const isRoomSelected = selectedConfigIds.includes(room.id);
                                return (`;
                                
const newRoomsMapStart = `{listing.rooms.map((room, idx) => {
                                const isRoomSelected = selectedConfigIds.includes(room.id);
                                return (
...
// We need to replace the old rooms map block exactly.
// Let's use a regex or string replacement to cut out the old `return (` to `)` that closes the map.
