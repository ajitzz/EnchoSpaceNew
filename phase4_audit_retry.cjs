...
const orig1 = `const { listing_id, title, description, video_url, media_urls, platforms, budget, target_locations, ad_format, feed_description, meta_pixel_id, meta_capi_token, google_conversion_id, google_conversion_label } = req.body;

    if (!listing_id || !title || !description) {
      return res.status(400).json({ error: 'listing_id, title, and description are required' });
    }`;

const repl1 = `const parseResult = campaignSchema.safeParse(req.body);
...
code = code.replace(orig1, repl1);

const orig2 = `const { title, description, video_url, media_urls, platforms, budget, status, target_locations, ad_format, feed_description, rejected_fields, meta_pixel_id, meta_capi_token, google_conversion_id, google_conversion_label } = req.body;`;

const repl2 = `const parseResult = campaignUpdateSchema.safeParse(req.body);
    if (!parseResult.success) {
      return res.status(400).json({ error: 'Invalid input', details: parseResult.error.errors });
    }
