...
code = code.replace(
  ...
  "UPDATE host_marketing_campaigns SET external_status_verified_at = NULL WHERE id = $1`, [testCampaignId]);\n    await pool.query(`UPDATE meta_publishing_transactions SET updated_at = NOW() - INTERVAL '1 hour' WHERE campaign_id = $1`, [testCampaignId]);"
);
